it('Kiểm tra thực hiện thành công khi dữ liệu là chữ tiếng Việt có dấu', function() {
    ...
    cy.get('body > div.modal.fade.ng-scope.ng-isolate-scope.in > div > div > form > div.modal-body > div > div:nth-child(1) > div:nth-child(2) > input').type('Tên có dấu');
    ...
    // test
    cy.get('body > div > div.page-container.ng-scope > div > div.page-content-wrapper > div > div > div > div.portlet-body > div.max-width-100-pc.scrollable > table > tbody > tr:nth-child(1) > td:nth-child(3)').should('contain', 'Tên có dấu');
});